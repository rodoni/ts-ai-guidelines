...
## Why It Matters
V8 and modern JS engines optimize property accesses via hidden classes (shapes / maps). Adding properties dynamically in different orders or using `delete` causes polymorphic transitions, deoptimizing inline caches into slow dictionary lookups. Critical in hot paths, parsers, and game engines, but unnecessary in ordinary UI/CRUD logic.
